...
public static class CategoricalDebugInitialSetup
{
    ...
    static CategoricalDebugInitialSetup()
    {
        ...
        if (EditorPrefs.GetBool(EditorPrefKey_OneTimePassDone, false))
            return;
        Debug.Log("Categorical Debug one-time-pass:  Assigning build define symbol "+ CatDebug.CONDITONAL_DEFINE_STRING + " in player build options.  Can be removed manually in player settings, or via toggle in Edit->Categorical Debug Settings");
